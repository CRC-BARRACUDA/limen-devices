...
#   dist/devices-<os>-<arch>.<ext>          the prebuilt library (the release asset)
#   dist/devices-<os>-<arch>.<ext>.sha256   its checksum (required by the manager)
...
name="devices"   # must match [lib] name in Cargo.toml (and `entry` in limen.toml)
